...
func main() {
	...
	flag.StringVar(&path, "path", "./", `文章路径`)
	flag.IntVar(&sleep, "sleep", 180, "发布间隔时间（秒）")
	flag.IntVar(&limit, "limit", 1000000, "限制文章发送数量")
	flag.Parse()
	...
	go func() {
		...
		for {
			// TODO: linux 没有目录下没有数据时 需要休眠一段时间在遍历
			// 打开目录
		...
		label1:
			...
			sleepSencond := 60
			if time.Now().Unix()-lastTimeStamp < int64(sleepSencond+sleep) {
				// 小于 睡眠时间 说明文件夹为空
				time.Sleep(time.Second * time.Duration(sleepSencond))
			}
			lastTimeStamp = time.Now().Unix()
